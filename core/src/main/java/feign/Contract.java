...
public interface Contract {
  ...
  abstract class BaseContract implements Contract {
    ...
    public MethodMetadata parseAndValidatateMetadata(Method method) {
      ...
      /**
       * 解决无注解式默认的提供method http call
       * @param data metadata
       * @param method method
       */
    protected  abstract void processNoAnnotationOnMethod(MethodMetadata data, Method method);
       ...
       * @param method method
       */
    protected abstract void processNoAnnotationsOnParameter(MethodMetadata data, Parameter method, int paramIndex);
    ...
    protected MethodMetadata parseAndValidateMetadata(Class<?> targetType, Method method) {
      ...
      if (method.getAnnotations().length == 0){
          processNoAnnotationOnMethod(data, method);
      } else {
      ...
      Parameter[] parameters = method.getParameters();
        for (int i = 0; i < count; i++) {
            ...
            if (parameterTypes[i] == URI.class) {
                ...
                processNoAnnotationsOnParameter(data, parameters[i], i);
...
//                data.bodyType(Types.resolve(targetType, targetType, method.getGenericParameterTypes()[i]));
  ...
  class Default extends BaseContract {
      ...
      @Override
      protected void processNoAnnotationOnMethod(MethodMetadata data, Method method) {
          return;
      }

      @Override
      protected void processNoAnnotationsOnParameter(MethodMetadata data, Parameter method, int paramIndex) {
        return;
      }
